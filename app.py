...
def main():
    ...
    st.title("Mulitple Image Encryption")
    ...
    if uploaded_images:
        ...
        if st.button("Encrypt and Decrypt Images"):
            ...
            for i, img_file in enumerate(uploaded_images):
                ...
                img = Image.open(img_file)  # Keep the image in RGB
...
def rna_encoding(Z, p10, u):
    ...
    # Reshape the bit planes to form a 4D array
...
def binary_matrix_to_decimal(matrix):
    # Reshape the matrix to a 3D array
...
def encryption_function(image):
    ...
    scrambled_bit_planes_4d = 1 - bit_planes_4d
    encoded_matrix = dna_encoding(scrambled_bit_planes_4d)
...
def decryption_function(image,encrypted_matrix,rna_translation_matrix):
    ...
    final_decrypted_matrix = binary_matrix_to_decimal(rescrambled_bit_planes_4d)
